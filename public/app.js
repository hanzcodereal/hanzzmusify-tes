...
const Library={
    render(){
        ...
        else{html+='<div class="grid grid-cols-2 gap-3">';pls.forEach(function(p){html+='<div onclick="Library.open(\''+p.id+'\')" class="bg-[#181818] rounded-xl p-3 cursor-pointer active:scale-95 hover:bg-[#282828] transition-colors"><img src="'+(p.image||FI)+'" class="w-full aspect-square object-cover rounded-lg mb-2" /><h3 class="font-bold text-sm truncate">'+p.name+'</h3><p class="text-[#6b7280] text-xs">'+p.songs.length+' lagu</p></div>';});html+='</div>';}
    ...
    open(id){
        ...
        else{html+='<div class="space-y-1">';pl.songs.forEach(function(s,i){html+='<div onclick="Library.playSong(\''+id+'\','+i+')" class="flex items-center gap-3 p-3 hover:bg-[#282828] rounded-lg cursor-pointer active:scale-[0.98] transition-colors"><img src="'+s.cover+'" class="w-10 h-10 rounded object-cover" /><div class="truncate"><p class="font-medium text-sm truncate">'+s.title+'</p><p class="text-[#6b7280] text-xs truncate">'+s.artist+'</p></div></div>';});html+='</div>';}
        html+='</div>';gid('view-library').innerHTML=html;lucide.createIcons();
    },
